...
public class SpigotDataType<P, C> implements DataType<P, C> {
    ...
    public static final SpigotDataType<IDataContainer, Location> LOCATION = of(IDataContainer.class, Location.class,
        (container, context) -> {
    ...
    SpigotDataType(Class<P> primitiveType, Class<C> complexType, BiFunction<P, DataAdapterContext, C> fromPrimitive,
    ...
    public static <P> SpigotDataType<P, P> of(Class<P> type) {
        return new SpigotDataType<>(type, type, (value, context) -> value, (value, context) -> value);
    }

    public static <P, C> SpigotDataType<P, C> of(Class<P> primitive, Class<C> complex, Function<P, C> from, Function<C, P> to) {
        return new SpigotDataType<>(primitive, complex, (value, context) -> from.apply(value), (value, context) -> to.apply(value));
    }

    public static <P, C> SpigotDataType<P, C> of(Class<P> primitive, Class<C> complex, BiFunction<P, DataAdapterContext, C> from,
        BiFunction<C, DataAdapterContext, P> to) {
        return new SpigotDataType<>(primitive, complex, from, to);
    }
