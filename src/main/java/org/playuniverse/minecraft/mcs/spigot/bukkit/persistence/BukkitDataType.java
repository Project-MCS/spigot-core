package org.playuniverse.minecraft.mcs.spigot.bukkit.persistence;
...
public class BukkitDataType<T, V> implements PersistentDataType<T, V> {
	...
	BukkitDataType(Class<T> primitiveType, Class<V> complexType, BiFunction<T, PersistentDataAdapterContext, V> fromPrimitive,
	...
	public static <T> BukkitDataType<T, T> of(Class<T> type) {
		return new BukkitDataType<>(type, type, (value, context) -> value, (value, context) -> value);
	}

	public static <T, V> BukkitDataType<T, V> of(Class<T> primitive, Class<V> complex, Function<T, V> from, Function<V, T> to) {
		return new BukkitDataType<>(primitive, complex, (value, context) -> from.apply(value), (value, context) -> to.apply(value));
	}

	public static <T, V> BukkitDataType<T, V> of(Class<T> primitive, Class<V> complex, BiFunction<T, PersistentDataAdapterContext, V> from,
		BiFunction<V, PersistentDataAdapterContext, T> to) {
		return new BukkitDataType<>(primitive, complex, from, to);
	}
