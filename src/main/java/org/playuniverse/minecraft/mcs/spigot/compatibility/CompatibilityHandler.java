...
public abstract class CompatibilityHandler {
	...
	public static class CompatAddon<E extends CompatibilityAddon> {
		...
		void start(PluginPackage pluginPackage) {
			...
			try {
				instance.onEnable(pluginPackage, SpigotCore.get());
			} catch (IncompatiblePluginException pluginException) {
				instance = null;
				SpigotCore.get().getPluginLogger().log(pluginException);
			} catch (Exception exception) {
				instance = null;
				SpigotCore.get().getPluginLogger().log("&bFailed to enable compatibility addon '&3" + owner.getSimpleName().split("\\.")[0] + "&7' for plugin '&3"
		...
		void shutdown() {
			if (instance == null)
				return;
			try {
				instance.onDisable(SpigotCore.get());
			} catch (Exception exception) {
				SpigotCore.get().getPluginLogger().log("&bFailed to disable compatibility addon " + owner.getSimpleName().split("\\.")[0] + "&7'!");
			}
			instance = null;
		}
